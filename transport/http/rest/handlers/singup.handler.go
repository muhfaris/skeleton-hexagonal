...
func SignUpHandler(rest app.RestApp) func(*fiber.Ctx) error {
	return func(ctx *fiber.Ctx) error {
		params := new(structures.SignUpRead)
		...
		if err := services.UserPublicService.SignUp(ctx.UserContext(), params); err != nil {
			return respond.Fail(
				ctx,
				respond.WithStatus(http.StatusInternalServerError),
				respond.WithError(err),
		...
		return respond.Success(ctx, respond.WithData("successfully created new account"))
	}
}
